<x-app-layout>
    ...
    <form action="{{ route('patients.appointments.update', $appointment) }}" method="post"
        ...
        <div class="col-span-full">
            ...
            @if (auth()->user()->role == 'patient')
                <div x-data="{
                    ...
                    <div class="flex items-center">
                        ...
                        <x-button onclick="copyToClipboard()" variant="primary">
                            {{ __('Salin') }}
                        </x-button>
                        <div id="copy-notification" class="ml-2 text-sm text-green-600 hidden">
            ...
            @else
                {{-- Konten lain untuk admin atau bidan, jika diperlukan --}}
        ...
        <div class="flex justify-end space-x-2 col-span-full">
            @if (in_array(auth()->user()->role, ['admin', 'bidan']))
                <x-button type="submit" variant="primary">
                    {{ __('Perbaharui status dan Kirim notifikasi') }}
                </x-button>
            @endif
